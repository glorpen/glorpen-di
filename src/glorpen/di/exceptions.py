...
class UnknownScopeException(ContainerException):
    """Raised when service has *scope* not assigned to :class:`glorpen.di.dic.Container` by :meth:`glorpen.di.dic.Container.set_scope_hierarchy`."""
...
class UnknownServiceException(ContainerException):
    """Raised when requesting service name which is not registered in :class:`glorpen.di.dic.Container`."""
...
class UnknownParameterException(ContainerException):
    """Raised when requesting parameter which is not registered in :class:`glorpen.di.dic.Container`."""
...
class ServiceAlreadyCreated(ContainerException):
    """Raised when service definition is changed but service is already created by :class:`glorpen.di.dic.Container`."""
    def __init__(self, svc_name):
